...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 基础属性 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
@export var base_recall_time: float = 3.0
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 当前属性 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
var recall_time: float = base_recall_time
...
func _update_parameters() -> void:
	...
	recall_timer.wait_time = recall_time
...
func tick_physics(state: State, delta: float) -> void:
	match state:
		...
		State.CHARGE:
			...
			var currentAngle := lerp_angle(rotation, targetAngle, rotation_speed * delta)
			# 更新对象的旋转角度
		...
		State.LANDING:
			position = position.move_toward(targetPos, projectile_speed*delta)
		State.RECALL:
			var targetDirection = slot.global_position - global_position
			rotation = lerp_angle(rotation, targetDirection.angle(), rotation_speed*delta)
