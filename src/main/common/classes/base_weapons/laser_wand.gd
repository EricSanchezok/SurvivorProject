...
@export var base_attack_speed: float = 1.0  #攻击速度
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 变量定义 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
@onready var bullets = {
	"normal_laser": preload("res://src/main/scene/role/weapons/Laser/normal_laser.tscn"),
}
var enemies: Array = []
...
func shoot() ->void:
	var now_bullet = bullets["normal_laser"].instantiate()
	...
	marker_2d.add_child(now_bullet)
