...
func get_next_state(state: State) -> int:
	if not target and state not in non_transferable_states and forward:
		finished = false
		return StateMachine.KEEP_CURRENT if state == State.BACKWARD else State.BACKWARD
	if global_position.distance_to(player.global_position) > furthest_distance and state not in non_transferable_states:
		finished = false
		return StateMachine.KEEP_CURRENT if state == State.BACKWARD else State.BACKWARD
	match state:
		State.WAIT:
