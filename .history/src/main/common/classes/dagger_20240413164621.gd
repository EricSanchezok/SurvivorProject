...
@export var base_furthest_distance: float = 150.0 # 离开玩家的最远距离
@export var base_attack_range: float = 80.0 # 自动索敌的范围
@export var base_attack_speed: float = 100.0
@export var base_attack_distance: float = 20.0 # 在敌人附近来回攻击的距离
...
func tick_physics(state: State, delta: float) -> void:
	match state:
		...
		State.ATTACK:
			...
			position = position.move_toward(targetPos, attack_speed*delta)
...
func beyond_distance() -> bool:
	return position.distance_to(player.global_position) > furthest_distance
