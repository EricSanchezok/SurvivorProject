...
# 计算考虑父节点速度后的调整移动步长
func calculate_adjusted_movement_step(direction: Vector2, base_step: float, delta: float) -> Vector2:
    var parentNode_velocity = get_parent().get_parent().velocity  # 假设祖父节点存在并有速度属性
    if not parentNode_velocity.is_zero_approx():
        var velocity_relative_to_parent = (direction * attack_speed) - parentNode_velocity
...
func tick_physics(state: State, delta: float) -> void:
	match state:
		...
		State.ATTACK:
			...
			move_towards_custom(targetPos, delta)
