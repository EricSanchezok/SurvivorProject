...
func perform_backswing(delta: float) -> void:
	...
	if totalTime > 0.01:
		global_position = Tools.move_towards_target(player, attack_backswing_speed, global_position, targetPos, delta)
		# move_toward_target(parentNode.global_position, delta)
	else:
		end_attack()

func move_toward_target(tP: Vector2, delta: float) -> void:
	'''
	移动武器朝向目标位置

	:param tP: Vector2 目标位置
	:param delta: float 时间间隔
	:return: void
	'''
	...
	if not isBezier:
		...
		global_position = global_position.move_toward(tP, movementStep)
	else:
		var movementStep = speed * delta
		...
		var t = 0.0
		while true:
			var nP := global_position.bezier_interpolate(startControlPoint, tP, tP, t)
			var distance := global_position.distance_to(nP)
			if distance > movementStep:
				break
			t += 0.01
		var nextPoint = global_position.bezier_interpolate(startControlPoint, tP, tP, t)
