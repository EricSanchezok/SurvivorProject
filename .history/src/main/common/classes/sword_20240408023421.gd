...
func _process(delta: float) -> void:
	if not is_attacking:
		...
		if target:
			...
			var move = direction * attack_speed * delta
			global_position += move
			if global_position.distance_to(target.global_position) < 10:
				target.take_damage(physical_attack_power, magic_attack_power)
				target.knockback(knockback, direction)
				is_attacking = false
				global_position = global_position - move
