...
func perform_windup(delta: float) -> void:
	...
	if totalTime > 0.01:
		move_toward_target(attack_windup_speed, target.global_position, delta)
	else:
...
func perform_backswing(delta: float) -> void:
	...
	if totalTime > 0.01:
		move_toward_target(attack_backswing_speed, parentNode.global_position, delta)
	else:
		end_attack()

func move_toward_target(speed: float, targetPos: Vector2, delta: float) -> void:
	if not isBezier:
		...
		while true:
			...
			var distance := global_position.distance_to(nextPoint)
			if distance > movementStep:
				break
			t += 0.01
		nextPoint = global_position.bezier_interpolate(startControlPoint, targetPos, targetPos, t)
