...
func tick_physics(state: State, delta: float) -> void:
	...
	match state:
		...
		State.DISAPPEAR:
			pass
		State.ATTACK:
			var localtargetPos = parentNode.to_local(targetPos)
			var localdir := (localtargetPos - position).normalized()
			rotation = lerp_angle(rotation, localdir.angle()-PI/2, rotation_speed)
			var movementStep = attack_speed * delta
			position = position.move_toward(localtargetPos, movementStep)
			if global_position.distance_to(localtargetPos) < 0.1:
				finished = true
