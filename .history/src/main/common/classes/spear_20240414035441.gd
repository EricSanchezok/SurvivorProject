...
@export var base_attack_wait_time: float = 1.5
...
func tick_physics(state: State, delta: float) -> void:
	match state:
		...
		State.LANDING:
			pass
		State.RECALL:
			var dir := (slot.global_position - position).normalized()
			rotation = lerp_angle(rotation, dir.angle(), rotation_speed*delta)
			position = position.move_toward(slot.global_position, projectile_speed*delta*1.2)
