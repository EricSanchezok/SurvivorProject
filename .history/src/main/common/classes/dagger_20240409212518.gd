...
@export var base_furthest_distance: float = 100.0
...
var non-transferable_states = [State.WAIT, State.CALCULATE, State.APPEAR, State.DISAPPEAR]
...
func get_next_state(state: State) -> int:
	...
	if global_position.distance_to(player.global_position) > furthest_distance and state != State.BACKWARD:
		finished = false
		return State.BACKWARD
	match state:
		State.WAIT:
