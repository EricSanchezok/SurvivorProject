...
# 	# 根据父节点的速度调整实际移动步长
# 	var adjusted_movement_step = calculate_adjusted_movement_step(player_velocity, direction_to_target, base_movement_step, delta)
...
func tick_physics(state: State, delta: float) -> void:
	match state:
		...
		State.ATTACK:
			...
			global_position = Tools.move_towards_target(player, attack_speed, targetPos, delta)
