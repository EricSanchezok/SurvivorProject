...
func tick_physics(state: State, delta: float) -> void:
	print(position.distance_to(slot.global_position))
	match state:
		State.WAIT:
...
func get_next_state(state: State) -> int:
	match state:
		State.WAIT:
			...
			if target and position.distance_to(slot.global_position) < 0.01:
				return State.ATTACK
		State.ATTACK:
			if not target or position.distance_to(target.global_position) < 0.01:
				return State.WAIT
