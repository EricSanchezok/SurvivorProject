...
func tick_physics(state: State, delta: float) -> void:
	if target:
		...
		print("position:", parentNode.to_local(targetPos))
		print("global_position:", targetPos)
		pass
	match state:
		State.WAIT:
		...
		State.ATTACK:
			...
			position = position.move_toward(parentNode.to_local(targetPos), attack_speed*delta)
			# global_position = Tools.move_towards_target(player, attack_speed, global_position, targetPos, delta)
			if position.distance_to(parentNode.to_local(targetPos)) < 0.1:
				finished = true
		State.FORWARD:
			pass
		State.BACKWARD:
