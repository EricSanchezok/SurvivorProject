...
func move_towards_custom(target_position: Vector2, movement_step: float) -> void:
	...
	if distance <= movement_step:
		...
		global_position += direction * movement_step  # 否则，向目标位置移动一个步长
...
func tick_physics(state: State, delta: float) -> void:
	match state:
		...
		State.DISAPPEAR:
			pass
		State.ATTACK:
			var parent_global_transform = parentNode.global_transform.basis_xform_inv(targetPos)
			var inv_parent_global_transform = parent_global_transform.affine_inverse()
			
			# 将目标全局位置转换为父节点局部坐标系中的位置
			var local_pos_for_target_global_pos = inv_parent_global_transform.xform(targetPos)

			# var targetDirection := local_pos_for_target_global_pos - position
			# rotation = lerp_angle(rotation, targetDirection.angle()-PI/2, rotation_speed)
			var movementStep = attack_speed * delta
			position = position.move_toward(local_pos_for_target_global_pos, movementStep)
