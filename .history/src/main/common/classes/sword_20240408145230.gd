...
func perform_windup(delta: float) -> void:
	...
	if totalTime > 0.01:
		move_toward_target(global_position, target.global_position, delta)
	else:
...
func perform_backswing(delta: float) -> void:
	...
	if totalTime > 0.01:
		move_toward_target(global_position, parentNode.global_position, delta)
	else:
		end_attack()

func move_toward_target(currentPos: Vector2, targetPos: Vector2, delta: float) -> void:
