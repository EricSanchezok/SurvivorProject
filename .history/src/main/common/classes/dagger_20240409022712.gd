...
func tick_physics(state: State, delta: float) -> void:
	match state:
		...
		State.ATTACK:
			...
			var local_pos_for_target_global_pos = inv_parent_global_transform.xform(target_global_position)
			
			# 设置子节点的局部位置，试图保持其全局位置不变
			position = local_pos_for_target_global_pos
			...
			move_towards_custom(targetPos, movementStep)
