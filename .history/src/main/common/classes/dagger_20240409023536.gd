...
func tick_physics(state: State, delta: float) -> void:
	match state:
		...
		State.DISAPPEAR:
			pass
		State.ATTACK:
			var parent_origin := parentNode.global_transform.inverse().get_origin()
			var parent_rotation := parentNode.global_transform.inverse().get_rotation()
			print("parent_origin: %s, parent_rotation: %s" % [parent_origin, parent_rotation])
