...
func tick_physics(state: State, delta: float) -> void:
	match state:
		State.WAIT:
			var dir = slot.global_position.direction_to(target.global_position)
			position = position.move_toward(slot.global_position, attack_backswing_speed*delta)
		State.ATTACK:
			rotation = lerp_angle(rotation, rotation_to(target.global_position), rotation_speed*delta)
...
func get_next_state(state: State) -> int:
	match state:
		State.WAIT:
			...
			if target and position.distance_to(slot.global_position) < 0.1:
				return State.ATTACK
		State.ATTACK:
			if not target or position.distance_to(target.global_position) < 0.1:
				return State.WAIT
