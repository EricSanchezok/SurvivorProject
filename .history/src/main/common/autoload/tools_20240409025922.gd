...
func move_towards_target(player: CharacterBody2D, speed: Vector2, current_position: Vector2, target_position: Vector2, delta: float) -> Vector2:
	# 计算目标方向和距离
...
# 计算考虑父节点速度后的调整移动步长
func calculate_adjusted_movement_step(parentVelocity:Vector2, speed: Vector2, direction: Vector2, base_step: float, delta: float) -> Vector2:
