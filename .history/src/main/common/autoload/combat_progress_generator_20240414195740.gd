...
func print_map(map):
    for i in range(map.size()):
        print("Layer", i, ":")
        for node in map[i]:
            var conn_ids = [map[i].index(x) for x in node.connections]  # 获取连接节点的索引
            print("  Node", map[i].index(node), "Type:", node.type, "Connections:", conn_ids)

# 在游戏脚本或场景的某个部分调用
var game_map = create_game_map(5, 3, 5)  # 创建一个示例地图
print_map(game_map)
