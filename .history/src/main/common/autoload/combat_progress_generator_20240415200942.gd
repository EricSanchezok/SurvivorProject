...
func connect_layers(prev_layer, next_layer):
    ...
    # Case 1: Only one node in the previous layer, connect to all nodes in the next layer
    ...
    # Case 4: Same number of nodes, one-to-one connection
        ...
        # Optionally: Randomly connect an additional node from prev_layer to a different node in next_layer
        ...
        for j in range(next_layer.size()):
            if j != rand_index:
                ...
                if distance < min_distance:
                    ...
                    closest_index = j
        if closest_index != -1:
            prev_layer[rand_index].connections.append(next_layer[closest_index])
