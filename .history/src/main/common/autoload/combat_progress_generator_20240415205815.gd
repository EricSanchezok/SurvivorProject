...
func determine_num_nodes(layer_index: int, total_layers: int, min_nodes_per_layer: int, max_nodes_per_layer: int, prev_layer_nodes: int) -> int:
	...
	if layer_index == first_layer or layer_index == last_layer:
		...
		# 0.8的概率和前一层数量差2
		if randf() < 0.8:
			var num_nodes = prev_layer_nodes + randi_range(-2, 2)
			if num_nodes < min_nodes_per_layer:
				num_nodes = min_nodes_per_layer
			elif num_nodes > max_nodes_per_layer:
				num_nodes = max_nodes_per_layer
			return num_nodes
