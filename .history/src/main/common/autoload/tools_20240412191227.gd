...
def success_check(success_rate):
    """
    Check if an action is successful based on the given success rate.
    
    Parameters:
    - success_rate (float): A float between 0 and 1 representing the success rate.
