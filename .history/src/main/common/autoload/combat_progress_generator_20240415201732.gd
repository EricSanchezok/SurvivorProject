...
func shuffle_and_select_indices(size, select_count):
	...
	indices.shuffle()
	return indices.slice(0, select_count - 1)

# Helper function to connect remaining nodes to the closest node
func connect_remaining_nodes(layer, other_layer, selected_indices, exclude_same_index=false):
	for i in range(layer.size()):
		if not selected_indices.has(i):
			var closest_index = -1
			var min_distance = layer.size() + other_layer.size()
			for j in range(other_layer.size()):
				...
				if distance < min_distance:
					...
					closest_index = j
			if closest_index != -1:
				layer[i].connections.append(other_layer[closest_index])
...
func connect_layers(prev_layer, next_layer):
	...
	# Case 3: More nodes in the next layer than in the previous layer
		...
		connect_remaining_nodes(next_layer, prev_layer, selected_indices)
	...
	# Case 4: Same number of nodes, one-to-one connection
		...
		connect_remaining_nodes(prev_layer, next_layer, shuffle_and_select_indices(prev_layer.size(), next_layer.size()), true)
