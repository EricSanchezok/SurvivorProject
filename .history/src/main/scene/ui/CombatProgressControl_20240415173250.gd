...
func draw_map(map) -> void:
	...
	for index_layer in max_layers:
		for index_node in max_nodes_in_layer:
			var container = container_matrix[index_layer][index_node]
			var node = map[index_layer][index_node]
			var color_rect = ColorRect.new()
			color_rect.size_flags_vertical = Control.SIZE_EXPAND_FILL
			color_rect.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			color_rect.rect_min_size = container_size
			container.add_color_override("background_color", type_colors[node.type])
