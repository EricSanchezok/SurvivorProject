...
func draw_map(map):
	...
	draw_connection(container_matrix[0][0], container_matrix[0][1])

func draw_connection(index_from: Vector2i, index_container_to: Vector2i):
	var container_from = container_matrix[index_from.x][index_from.y]
	var container_to = container_matrix[index_container_to.x][index_container_to.y]
	var pos_from = index_from * container_from.size
	var pos_to = index_container_to * container_to.size
	var line = Line2D.new()
	line.width = 10
	line.default_color = Color(1, 1, 1)  # 设置线的颜色
	print(container_matrix.find(container_from))
	print(container_from.global_position)
	print(container_to.global_position)
	# 计算两个容器中心的坐标
