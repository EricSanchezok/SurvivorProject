...
func draw_map(map):
	...
	draw_connection(Vector2i(0, 0), Vector2i(1, 1))
...
func draw_connection(index_from: Vector2i, index_to: Vector2i):
	...
	var pos_from = container_from.global_position + container_size / 2
	var pos_to = container_to.global_position + container_size / 2
	var line = Line2D.new()
	line.width = 10
