...
func draw_map(map) -> void:
	...
	for index_layer in max_layers:
		for node in map[index_layer]:
			...
			var container = container_matrix[index_layer][index_container]
			...
			for connection in node.connections:
				...
				draw_connection(Vector2i(index_layer, index_container), Vector2i(index_layer+1, next_index))
...
func draw_connection(index_from: Vector2i, index_to: Vector2i) -> void:
	...
	var pos_from = Vector2(index_from.x * container_size.x, index_from.y * 56) + container_size / 2
	var pos_to = Vector2(index_to.x * container_size.x, index_to.y * 56) + container_size / 2
