...
func _ready() -> void:
	...
	combat_node.connect("selected_changed", _on_toggled)
...
func _on_activate_changed(activate: bool) -> void:
	print("activate changed")
	disabled = not activate

func _on_toggled(toggled_on: bool) -> void:
	print(Engine.get_physics_frames(), "  selected changed")
	if toggled_on and not combat_node.selected:
		combat_node.selected = true
		CombatRouteManager.update(combat_node)
