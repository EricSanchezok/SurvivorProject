...
func get_container_position(index_layer: int, index_node: int) -> Vector2:
	...
	var per_index_length = h_box_container.size.y / container_matrix[index_layer].size()
	var pos_y = index_node * per_index_length
...
func draw_connection(index_from: Vector2i, index_to: Vector2i) -> void:
	...
	var pos_from = Vector2(index_from.x * container_size.x, index_from.y * 56) + container_from.size / 2
