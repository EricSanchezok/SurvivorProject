...
func get_container_position(index_layer: int, index_node: int) -> Vector2:
	...
	print("container_matrix[index_layer].size(): ", container_matrix[index_layer].size())
	print("h_box_container.size.y: ", h_box_container.size.y)
	var pos_y = index_node * h_box_container.size.y / container_matrix[index_layer].size()
	print("pos_x: ", pos_x)
	print("pos_y: ", pos_y)
	return Vector2(pos_x, pos_y) + Vector2(container_size.x / 2, h_box_container.size.y / container_matrix[index_layer].size() / 2)
