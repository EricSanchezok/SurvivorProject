...
func init_slots() -> void:
	...
	for i in range(5, 10):
		slots[i].position = Vector2(
			cos(deg_to_rad(72 * (i - 5))) * slot_radius,
			sin(deg_to_rad(72 * (i - 5))) * slot_radius,
		)

func move_slots(delta) -> void:
	#前五个采用正弦的形式上下浮动
	for i in range(0, 5):
		slots[i].position.y = slot_height + sin(current_time + i * 0.5) * 5.0
	#后五个采用圆周运动
	for i in range(5, 10):
		slots[i].position = Vector2(
			cos(deg_to_rad(72 * (i - 5) + current_time * slot_speed_rotation)) * slot_radius,
			sin(deg_to_rad(72 * (i - 5) + current_time * slot_speed_rotation)) * slot_radius,
		)
	current_time += delta
