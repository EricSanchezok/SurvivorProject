...
func get_random_postion_in_shape(shape: CollisionShape2D) -> Vector2:
	if shape is CircleShape2D:
		var circle = shape as CircleShape2D
		var radius = circle.radius
		var angle = randf() * 2 * PI
		return Vector2(cos(angle), sin(angle)) * radius + circle.position
	elif shape is RectangleShape2D:
		var rect = shape as RectangleShape2D
		var x = randf() * rect.extents.x * 2 - rect.extents.x
		var y = randf() * rect.extents.y * 2 - rect.extents.y
		return Vector2(x, y) + rect.position
    else:
